...
def test_export_import_round_trip_excludes_model_secrets(tmp_path):
    ...
    try:
        run = source_repo.create_run(LearningRunCreate(keyword="RAG", mode="light"))
        ...
        node = source_repo.upsert_node(KnowledgeNodeCreate(type="keyword", name="RAG"))
        other = source_repo.upsert_node(KnowledgeNodeCreate(type="concept", name="Retrieval"))
        ...
        exported = export_knowledge(source_repo)
